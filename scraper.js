...
const scrapeTrades = async (f1, f2, tradeTableIndex = 0) => {
  ...
  $(selector, html).not('.thead').each(function () {

    const child = (index) => $(this).children(`td:nth-child(${index})`).text();
    const suffix = $(this).children(`td:nth-child(1)`).children('a').attr('href');
